  ...
  return (
    <>
      <div style={{ display: 'grid', gap: '0.5rem', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))' }}>
        {orders?.map((order) => <OrderCard order={order} header={order.username ?? 'Unknown'} onEditPrice={(order) => setEditOrderId(order.id)} key={order.id} />
        )}
      </div>
      <Dialog onClose={() => setEditOrderId('')} open={!!editOrderId}>
        <DialogTitle>Edit Price</DialogTitle>
        {!!editOrderId && <EditPriceForm orderId={editOrderId} onClose={() => setEditOrderId('')} />}
      </Dialog>
    </>
  );
}
